...
//#region node_modules/.nitro/vite/services/ssr/assets/router-DRjGua5C.js
var router_DRjGua5C_exports = /* @__PURE__ */ __exportAll({
	_: () => PLUGINS,
	a: () => getBaseURL,
	b: () => buildStaticMarkets,
	c: () => equityOf,
	d: () => formatPct,
	f: () => formatQty,
	g: () => GALLERY,
...
var styles_default = "/assets/styles-DB6zDk-K.css";
var APP_NAME = "Aether";
var Route$11 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "Aether is a local-first AI finance agent. Models, skills, plugins, execution — plus ACP for your own code agent."
			},
			{
			...
			},
			{
				...
				content: "AI finance agent. The brain, not the vault."
			},
			...[]
		],
		links: [
...
var $$splitComponentImporter$7 = () => import("./routes-Cge4NQ9I.mjs");
var Route$10 = createFileRoute("/")({ component: lazyRouteComponent($$splitComponentImporter$7, "component") });
var $$splitComponentImporter$6 = () => import("./accounts-DYCgjCQ9.mjs");
var Route$9 = createFileRoute("/accounts")({ component: lazyRouteComponent($$splitComponentImporter$6, "component") });
var $$splitComponentImporter$5 = () => import("./agents-Czj9JG1H.mjs");
var Route$8 = createFileRoute("/agents")({ component: lazyRouteComponent($$splitComponentImporter$5, "component") });
var $$splitComponentImporter$4 = () => import("./login-DI11vDMG.mjs");
var Route$7 = createFileRoute("/login")({ component: lazyRouteComponent($$splitComponentImporter$4, "component") });
var $$splitComponentImporter$3 = () => import("./models-BIBfJd9k.mjs");
var Route$6 = createFileRoute("/models")({ component: lazyRouteComponent($$splitComponentImporter$3, "component") });
var $$splitComponentImporter$2 = () => import("./plugins-_yQwnI-Y.mjs");
var Route$5 = createFileRoute("/plugins")({ component: lazyRouteComponent($$splitComponentImporter$2, "component") });
var $$splitComponentImporter$1 = () => import("./skills-BG7794gf.mjs");
var Route$4 = createFileRoute("/skills")({ component: lazyRouteComponent($$splitComponentImporter$1, "component") });
var $$splitComponentImporter = () => import("./trade-DG0snuiF.mjs");
var Route$3 = createFileRoute("/trade")({ component: lazyRouteComponent($$splitComponentImporter, "component") });
...
var PREDICTS = [
	...
	{
		symbol: "FED-CUT",
		name: "Fed cuts next meeting",
		base: .38
	},
	{
		symbol: "ETH-ETF-FLOW",
		name: "ETH ETF net inflow this month",
		base: .57
	}
];
function hash32(s) {
...
function seededWalk(seed, now = Date.now(), steps = 48) {
	const bucket = Math.floor(now / 4e3);
	const out = [];
	let x = 1;
	for (let i = 0; i < steps; i++) {
		const delta = (hash32(`${seed}:${bucket - (steps - i)}`) % 1e3 - 500) / 9e3;
		x = Math.max(.2, x * (1 + delta));
		out.push(x);
	}
	return out;
}
function seedPrice(base, seed, now = Date.now()) {
	const walk = seededWalk(seed, now, 24);
	return base * walk[walk.length - 1];
}
function seedChange(seed, now = Date.now()) {
	return (hash32(`${seed}:chg:${Math.floor(now / 6e4)}`) % 1600 - 700) / 100;
}
function sparkFrom(base, seed, now) {
	return seededWalk(seed, now).map((m) => base * m);
}
function buildStaticMarkets(now = Date.now()) {
	const spots = CRYPTO.map((c) => {
		...
		return {
			...
			price,
			change24h,
			volume24h: c.base * 1e6 * (.4 + hash32(c.symbol) % 80 / 100),
			high24h: price * 1.018,
			low24h: price * .982,
			spark
		};
	});
	const perps = CRYPTO.slice(0, 5).map((c) => {
		...
		return {
			...
			name: `${c.name} Perp`,
			venue: "perp",
			price: m.price * (1 + seedChange(`${c.symbol}-p`, now) / 1e4)
		};
	});
	const eqs = EQUITIES.map((e) => {
		...
		return {
			...
			volume24h: e.base * 8e4,
			high24h: price * 1.012,
			low24h: price * .988,
			spark: sparkFrom(e.base, e.symbol, now)
		};
	});
	const preds = PREDICTS.map((p) => {
		const price = Math.min(.97, Math.max(.03, seedPrice(p.base, p.symbol, now)));
		return {
			symbol: p.symbol,
			name: p.name,
			venue: "predict",
			...
			volume24h: 24e5 + hash32(p.symbol) % 8e5,
			high24h: Math.min(.99, price + .04),
			low24h: Math.max(.01, price - .04),
			spark: sparkFrom(p.base, p.symbol, now).map((v) => Math.min(.97, Math.max(.03, v)))
		};
	});
	return [
		...spots,
...
var COINGECKO_IDS = Object.fromEntries(CRYPTO.map((c) => [c.symbol, c.id]));
var SKILLS = [
	{
		id: "druckenmiller",
		...
		era: "1981–",
		summary: "Top-down liquidity regimes. Concentrate when the tape agrees. Cut losers immediately.",
		signals: [
			"Dollar liquidity",
			"Real rates impulse",
	...
	},
	{
		...
		signals: [
			"Mindshare acceleration",
			"Funding extremes",
			"Relative L1 strength"
		],
	...
	},
	{
		...
		signals: [
			"Crowd certainty",
			"Feedback between price and story",
...
var PLUGINS = [
	{
		...
		name: "Market Data",
		kind: "market",
		blurb: "Spot, perp, and equity marks. CoinGecko for majors, seeded tape for the rest.",
		source: "CoinGecko + Aether tape"
	},
	{
		id: "onchain",
		name: "On-chain",
...
var g = globalThis;
function cache$1(slot, ttl) {
	if (!slot) return null;
	if (Date.now() - slot.at > ttl) return null;
	return slot.value;
}
async function getMarkets() {
	const hit = cache$1(g.__aetherMarkets, 12e3);
	if (hit) return hit;
	const base = buildStaticMarkets();
	try {
		const ids = Object.values(COINGECKO_IDS).join(",");
		const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${ids}&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true`, { headers: { accept: "application/json" } });
		if (res.ok) {
			const json = await res.json();
			for (const m of base) {
				const cg = m.coingeckoId ? json[m.coingeckoId] : void 0;
				if (!cg?.usd) continue;
				m.price || cg.usd;
				m.price = cg.usd;
				...
				if (m.symbol.endsWith("-PERP")) {}
			}
			for (const m of base) {
				if (!m.symbol.endsWith("-PERP")) continue;
...
async function getCandles(symbol) {
	g.__aetherCandles ??= /* @__PURE__ */ new Map();
	const hit = cache$1(g.__aetherCandles.get(symbol), 6e4);
	if (hit) return hit;
	const m = (await getMarkets()).find((x) => x.symbol === symbol);
	const cgId = m?.coingeckoId ?? COINGECKO_IDS[symbol.replace("-PERP", "")];
	if (cgId) try {
		const res = await fetch(`https://api.coingecko.com/api/v3/coins/${cgId}/market_chart?vs_currency=usd&days=1`, { headers: { accept: "application/json" } });
		if (res.ok) {
			...
			for (const [t, p] of prices) {
				...
				if (!last || last.t !== key) grouped.push({
					t: key,
					o: p,
					h: p,
					l: p,
	...
	const spark = m?.spark ?? [];
	const now = Date.now();
	const candles = spark.map((c, i) => {
		const prev = spark[i - 1] ?? c;
		return {
			t: now - (spark.length - i) * 15 * 60 * 1e3,
			o: prev,
			h: Math.max(prev, c) * 1.002,
			l: Math.min(prev, c) * .998,
			c
		};
	});
	if (!candles.length) candles.push({
		t: now,
		o: px,
		h: px,
		l: px,
...
function onchainSnapshot(symbol) {
	...
	return {
		symbol: id,
		funding8h: Number((1e-4 + wobble * 4e-4).toFixed(5)),
		openInterestUsd: Math.round(12e8 + n * 8e6 + wobble * 8e7),
		stablecoin7dPct: Number((1.2 + wobble * 2.4).toFixed(2)),
		exchangeNetflow: wobble > 0 ? "outflow" : "inflow",
		comment: wobble > .3 ? "Funding elevated; late long squeeze risk." : wobble < -.3 ? "Funding compressed; squeeze fuel if spot leads." : "Funding mid-range. No crowding extreme."
	};
}
var WIRE = [
	{
		tag: "BTC",
		text: "Spot desks report steady bid into the US cash open; basis unremarkable."
	},
	{
		tag: "ETH",
		text: "Staking queue stable. Pair traders still fade ETHBTC strength above local range."
	},
	{
		tag: "SOL",
		text: "App-token volume clustered around SOL beta; funding on SOL-PERP slightly rich."
	},
	{
		tag: "HYPE",
		text: "Perp venue race still the narrative. Open interest concentrated in the front month."
	},
	{
		tag: "NVDA",
		text: "Semiconductor complex following a quiet bid. No new supply-chain print today."
	},
	{
		tag: "FED",
		text: "Rates market still two-sided into the next decision. Cuts are not a consensus lock."
	},
	{
		tag: "LIQ",
		text: "Net dollar liquidity still the cleaner input than any single CPI print."
	},
	{
		tag: "MACRO",
		text: "Cross-asset correlation rose overnight. Treat single-name stories as second."
	}
];
function newsWire(query) {
...
var Route$2 = createFileRoute("/api/markets")({ server: { handlers: { GET: async ({ request }) => {
	const symbol = new URL(request.url).searchParams.get("candles");
	if (symbol) {
...
function formatUsd(n, digits = 2) {
	...
	const d = abs >= 1e3 ? 2 : abs >= 1 ? 2 : abs >= .01 ? 4 : 6;
...
var WebsocketTransport = class {
	...
	constructor(url) {
		...
		this.ready = new Promise((resolve, reject) => {
			...
			this.ws.addEventListener("error", () => reject(/* @__PURE__ */ new Error(`ACP websocket failed: ${url}`)));
		});
	...
	send(msg) {
		this.ready.then(() => this.ws.send(JSON.stringify(msg)));
	}
	onMessage(cb) {
		this.handlers.push(cb);
	}
...
var LoopbackAgent = class {
	runPrompt;
	emit;
	sessions = /* @__PURE__ */ new Set();
	constructor(runPrompt, emit) {
	...
	async handle(msg) {
		...
		try {
			...
			if ("id" in req) this.reply(req.id, void 0, {
				code: -32601,
...
var AcpClient = class AcpClient {
	...
	static async connect(opts, runPrompt) {
		let transport;
		...
		} else transport = new LoopbackTransport(runPrompt);
		...
		await client.request("initialize", {
			...
			clientInfo: {
				name: "Aether",
				title: "Aether Finance Harness",
				version: "1.0.0"
			}
		});
		client.sessionId = (await client.request("session/new", {
	...
	request(method, params) {
		...
		return new Promise((resolve, reject) => {
			const timer = setTimeout(() => {
				...
				reject(/* @__PURE__ */ new Error(`ACP timeout on ${method}`));
			}, 12e4);
			this.pending.set(id, {
				resolve: (v) => {
					clearTimeout(timer);
	...
	inbound(msg) {
		this.traces.push({
			t: Date.now(),
			dir: "in",
			method: "method" in msg ? msg.method : void 0,
			raw: msg
		});
		...
		if ("id" in msg && this.pending.has(msg.id)) {
			...
			else p.resolve("result" in msg ? msg.result : void 0);
		}
	}
};
var clientCache = /* @__PURE__ */ new Map();
function acpCacheKey(opts) {
	return `${opts.transport}::${opts.command ?? ""}::${opts.url ?? "loopback"}`;
}
async function getSharedAcpClient(key, connect) {
...
var FINANCE_TOOLS = [
	...
	},
	{
		...
		function: {
			name: "propose_trade",
			description: "Propose a paper order. The harness executes it if the book allows. Always include a reason.",
			parameters: {
				type: "object",
				properties: {
					symbol: { type: "string" },
...
function buildSystemPrompt(input) {
	...
	const tape = input.markets.slice(0, 24).map((m) => `${m.symbol.padEnd(12)} ${m.price.toPrecision(6)}  ${m.change24h >= 0 ? "+" : ""}${m.change24h.toFixed(2)}%  ${m.venue}`).join("\n");
...
- Positions: ${input.book.positions.length ? input.book.positions.map((p) => `${p.side} ${p.qty} ${p.symbol} @ ${p.avgPrice} x${p.leverage}`).join("; ") : "flat"}
...
function toAcpPrompt(system, history, user) {
	const hist = history.slice(-12).map((m) => `${m.role.toUpperCase()}: ${m.content}`).join("\n\n");
	return [
		{
			type: "text",
			text: user
...
function markPrice(markets, symbol) {
	return markets.find((m) => m.symbol === symbol)?.price ?? 0;
}
function positionPnl(pos, px) {
	const dir = pos.side === "long" ? 1 : -1;
	return (px - pos.avgPrice) * pos.qty * dir;
}
function equityOf(cash, positions, markets) {
	let eq = cash;
...
function applyTrade(book, markets, trade) {
	...
	if (existing && existing.side === "long") {
		...
		const positions = leftover <= 1e-10 ? book.positions.filter((p) => p.id !== existing.id) : book.positions.map((p) => p.id === existing.id ? {
			...p,
			qty: leftover
		} : p);
		...
		if (remainder > 1e-10) {
			if (venue !== "perp") return {
				book: next,
				order,
				fill
			};
			return applyTrade(next, markets, {
				...trade,
				qty: remainder,
				side: "sell",
				reason: trade.reason
	...
	if (venue !== "perp" && venue !== "predict") return {
		book,
		order: ghostOrder(trade, "cancelled"),
...
function crosses(side, limit, mark) {
	return side === "buy" ? limit >= mark : limit <= mark;
}
function openOrAdd(book, symbol, venue, side, qty, px, leverage) {
	...
	return {
		...book,
		positions: book.positions.map((p) => p.id === existing.id ? {
			...p,
			qty: total,
			avgPrice: avg
		} : p)
...
function ghostOrder(trade, status) {
	return {
		id: uid("ord"),
		symbol: trade.symbol,
...
function filledOrder(trade, venue, px, leverage) {
	const now = Date.now();
	return {
		id: uid("ord"),
		symbol: trade.symbol,
...
function makeFill(order, px, fee) {
	return {
		id: uid("fil"),
		orderId: order.id,
		symbol: order.symbol,
...
function coerceTrade(raw) {
	...
	return {
		...
		qty,
		price: typeof o.price === "number" ? o.price : void 0,
		leverage: typeof o.leverage === "number" ? o.leverage : void 0,
		reason: String(o.reason ?? o.thesis ?? "")
	};
}
async function runTurn(input, emit) {
	const markets = await getMarkets();
	...
	if (input.modelId.startsWith("acp") || input.modelId === "acp-loopback") {
		await runAcpTurn(input, system, markets, emit);
		return;
	}
	await runGrokTurn(input, system, markets, emit);
}
async function runGrokTurn(input, system, markets, emit) {
	const messages = [
		{
			role: "system",
			content: system
	...
	if (lastText) {
		...
		for (const t of tryParseTrades(lastText)) emit({
			type: "trade",
			trade: t
		});
	}
...
async function runAcpTurn(input, system, markets, emit) {
	const cfg = input.acp ?? {
		id: "loopback",
		name: "ACP Loopback",
		transport: "loopback"
	};
	const runPrompt = async ({ sessionId, prompt, emit: acpEmit }) => {
		const userText = prompt.filter((b) => b.type === "text").map((b) => b.type === "text" ? b.text : "").join("\n");
		const messages = [{
			role: "system",
			content: system
		}, {
			role: "user",
			content: userText || input.text
		}];
		let guard = 0;
		let last = "";
		while (guard++ < 6) {
		...
		if (last) {
			const chunk = 80;
			for (let i = 0; i < last.length; i += chunk) acpEmit({
				sessionUpdate: "agent_message_chunk",
				content: {
					type: "text",
					text: last.slice(i, i + chunk)
				}
			});
		}
	...
	try {
		const client = await getSharedAcpClient(acpCacheKey({
			...
			onUpdate: (sessionId, update) => {
				if (update.sessionUpdate === "agent_message_chunk") emit({
					type: "token",
					text: update.content.text
				});
				else if (update.sessionUpdate === "agent_thought_chunk") emit({
					type: "thought",
					text: update.content.text
				});
				else if (update.sessionUpdate === "tool_call") emit({
					type: "tool",
					...
					const text = update.content?.map((c) => c.content.text).join("\n");
					emit({
						type: "tool",
						id: update.toolCallId,
						name: "tool",
		...
		emit({
			type: "acp",
			sessionId: client.getSessionId() ?? void 0,
		...
		emit({
			type: "acp",
			sessionId: client.getSessionId() ?? void 0,
			method: result.stopReason
		});
		emit({ type: "done" });
	} catch (err) {
		emit({
			type: "error",
			message: err instanceof Error ? err.message : "ACP agent failed. Check the command, or use Grok / ACP Loopback."
		});
		emit({ type: "done" });
	}
}
async function execTool(name, rawArgs, ctx) {
	...
	if (name === "list_markets") {
		...
		const rows = ctx.markets.filter((m) => !venue || m.venue === venue).map((m) => ({
			symbol: m.symbol,
			venue: m.venue,
			price: m.price,
			change24h: m.change24h
	...
	if (name === "get_book") return JSON.stringify(ctx.book);
	if (name === "get_onchain") return JSON.stringify(onchainSnapshot(symbol || "BTC"));
	if (name === "get_news") return JSON.stringify(newsWire(String(args.query ?? symbol ?? "MACRO")));
	if (name === "get_candles") {
		const candles = await getCandles(symbol || "BTC");
		return JSON.stringify(candles.slice(-40));
	}
	if (name === "propose_trade") {
		const trade = {
			...
			qty: Number(args.qty),
			price: typeof args.price === "number" ? args.price : void 0,
			leverage: typeof args.leverage === "number" ? args.leverage : void 0,
		...
		if (result.error) return JSON.stringify({
			ok: false,
			error: result.error
		});
...
var Route$1 = createFileRoute("/api/agent/turn")({ server: { handlers: { POST: async ({ request }) => {
	const body = await request.json();
	if (!body?.text || typeof body.text !== "string") return Response.json({ error: "Missing text" }, { status: 400 });
	const text = body.text.slice(0, 8e3);
	return sseResponse((emit) => runTurn({
		...body,
		text
	}, emit));
} } } });
...
async function createPgliteSql() {
	...
	const migrate = async () => {
		const migrations = /* #__PURE__ */ Object.assign({
			...
			"/migrations/0002_harness.sql": _0002_harness_default
		});
...
var rootRouteChildren = {
	...
	AccountsRoute: Route$9.update({
		id: "/accounts",
		path: "/accounts",
		getParentRoute: () => Route$11
	}),
	AgentsRoute: Route$8.update({
		id: "/agents",
		path: "/agents",
		getParentRoute: () => Route$11
	}),
	...
	ModelsRoute: Route$6.update({
		id: "/models",
		path: "/models",
		getParentRoute: () => Route$11
	}),
	PluginsRoute: Route$5.update({
		id: "/plugins",
		path: "/plugins",
		getParentRoute: () => Route$11
	}),
	SkillsRoute: Route$4.update({
		id: "/skills",
		path: "/skills",
		getParentRoute: () => Route$11
	}),
	TradeRoute: Route$3.update({
		id: "/trade",
		path: "/trade",
		getParentRoute: () => Route$11
	}),
	ApiMarketsRoute: Route$2.update({
		id: "/api/markets",
		path: "/api/markets",
		getParentRoute: () => Route$11
	}),
	ApiAgentTurnRoute: Route$1.update({
		id: "/api/agent/turn",
		path: "/api/agent/turn",
		getParentRoute: () => Route$11
	}),
...
//#endregion
export { getBaseURL as _, PACKAGE_VERSION as a, uid as b, STARTING_CASH as c, cn as d, emptyBook as f, formatUsd as g, formatQty as h, GROK_PROVIDERS as i, applyTrade as l, formatPct as m, GALLERY as n, PLUGINS as o, equityOf as p, GENERIC_OAUTH_ERROR_CODES as r, SKILLS as s, BUILTIN_MODELS as t, buildStaticMarkets as u, router_DRjGua5C_exports as v, venueLabel as x, snapshotOf as y };
